...
export class CinematicEngine {
    ...
    async startReveal(options = {}) {
        try {
            ...
            let musicAnalysis = { tempo: 'medium', tempoValue: 120, intensity: 'moderate', intensityValue: 0.5, mood: 'heroic' };
            if (audio) {
                try {
            ...
            // Store music analysis for camera updates
            this.currentMusicAnalysis = musicAnalysis;
            ...
            // Calculate exact duration based on BPM (e.g., 32 beats = 8 bars of 4/4 time)
            const beatsToPlay = 32;
            const durationInSeconds = (60 / musicAnalysis.tempoValue) * beatsToPlay;
            this.duration = durationInSeconds || this.currentSequence.totalDuration || 15;
    ...
    updateCamera(progress, phase) {
        ...
        const cameraState = sequence.getCameraState(progress, this.modelCenter, this.modelSize, this.currentMusicAnalysis);
